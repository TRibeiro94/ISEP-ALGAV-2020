...
getLines():-
    ...
    retractall(line(_,_,_,_)),
        ...
        assert(node('Baltar', 'BALTR', true, false, 41.1937898023744, -8.38716802227697)),
        assert(node('Besteiros', 'BESTR', false, false, 41.217018845589, -8.34043029659082)),
        assert(node('Cete', 'CETE', true, false, 41.183243425797, -8.35164059584564)),
        assert(node('Duas Igrejas', 'DIGRJ', false, false, 41.2278665802794, -8.35481024956726)),
        assert(node('Gandra', 'GAND', false, false, 41.1956579348384, -8.43958765792976)),
        assert(node('Mouriz', 'MOURZ', true, false, 41.1983610215263, -8.36577272258403)),
        assert(node('Recarei', 'RECAR', false, false, 41.1599363478137, -8.42215867462191)),
        assert(node('Vandoma', 'VANDO', true, false, 41.2328015719913, -8.34160692293342)),
        assert(node('Vila Cova de Carros', 'VCCAR', true, false, 41.2090666564063, -8.35109395257277)),
        assert(node('Casa da Musica', 'CDMUS', true, false, 41.20904032564063, -8.3710423395257277)),
        assert(node('Pacos de Ferreira', 'PADFE', true, false, 41.2432032564063, -8.3534423395257277)),
        assert(node('Trindade', 'TRIND', true, false, 41.26344032564063, -8.3623453395257277)),
        assert(line('Path:45',45,['ESTPA','PADFE','CDMUS','TRIND','MOURZ'],[])),
        assert(line('Path:88',88,['CDMUS','PADFE','RECAR','CRIST','ESTPA'],[])),
        assert(line('Path:90',90,['VANDO','TRIND','CDMUS','DIGRJ','BESTR'],[])),
...
getDistance(_, pathNodes([]),_):-print("Couldn't get distance for pathNode"),!.
...
getNodeID(_,pathNodes([]),_):-print("Couldn't get node IDs"),!.
...
procurarHora( _, [], _, _):- print("ERRO: Não foi encontrada uma hora de partida a partir da hora indicada!").
...
postSolution(StartingNode,EndNode,LeavingTime,ArrivingTime,FullPath):-
        ...
        http_post('http://localhost:6700/closestPath', json(Term), _, []).
