...
inicializa:-write('Numero de novas Geracoes: '),read(NG), 			
    ...
    write('Numero de Geracoes ate Estabilizacao:'), read(G),
...
postSolution(H,V):-
    ...
    http_post('https://arqsi2020-grupo67.herokuapp.com/geneticSolutions', json(Term), _, []).
...
avalia(Seq,V):-
	...
	lista_motoristas_nworkblocks(_,ListaMot),
...
restricaoHorarios([(Inicio,Fim,Mot)|Tail],L,PenalizacaoHorarios):-
	...
	;	
		(nth0(_,L,(InicioCont,FimCont,Mot)),
		(
			(Inicio<InicioCont,PenInicioAntesCont is InicioCont-Inicio)
		;	
			(PenInicioAntesCont is 0)
		),
		(
			(Fim>FimCont,PenFimDepoisCont is Fim-FimCont)
		;	
			(PenFimDepoisCont is 0)
		),
		PenalizacaoHorarios is PenalizacaoHorariosResto + PenFimDepoisCont + PenInicioAntesCont)
	),!.
...
gera_geracao(_,_,Ti,Tlim,Pop,_,_,_):-get_time(Tf),TPassado is Tf-Ti,print("Tempo passado: "),print(TPassado),nl, TPassado>=Tlim,set_fittest(Pop),!.
gera_geracao(G,G,_,_,Pop,_,_,_):-!,write('Geracao '), write(G), write(':'), nl, write(Pop), nl,media_populacao(Pop,0,Media),print("Media populacao: "),print(Media),nl,set_fittest(Pop).
gera_geracao(N,_,_,_,[H1*P1|T1],AvEspecifica,_,_):- P1=<AvEspecifica,write('Geracao '), write(N), write(':'), nl, write([H1*P1|T1]), nl,set_fittest([H1*P1|T1]),!.
gera_geracao(N,_,_,_,[H1*P1|T1],_,Gest,Gest):- write('Geracao '), write(N), write(':'), nl, write([H1*P1|T1]), nl,set_fittest([H1*P1|T1]),!.

gera_geracao(N,G,Ti,Tlim,Pop,AvEspecifica,Gest,Count):-
	write('Geracao '), write(N), write(':'), nl, write(Pop), nl,
	media_populacao(Pop,0,Media),
...
kill_unworthy2([],_,_,PopTemp,PopTemp):-print("Perigo, especie em extincao!").
